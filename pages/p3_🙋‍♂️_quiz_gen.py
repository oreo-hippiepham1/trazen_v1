...
from rag.quiz_agent import get_quiz_agent
...
st.title("Let's Generate The Quizzes!")

if ('selected_chapter' not in st.session_state) or (st.session_state['selected_chapter'] is None):
...
if __name__ == "__main__":
    # Initialize the QuizGen class
    quiz_gen = QuizGen(namespace=st.session_state['selected_chapter']['id'])

    if 'quiz_bank' not in st.session_state:
        st.session_state['quiz_bank'] = None

    st.header("Quiz Generation Logs")
    gen = st.button(f"Generate Quiz Bank for chapter {st.session_state['selected_chapter']['id']}")
    if gen:
        # Check if the quiz bank is already generated
        if st.session_state['quiz_bank'] is not None:
            st.warning("Quiz bank already generated. Please reload to generate a new one.")
        else:
            # Generate quiz bank
            with st.spinner("Generating..."):
                quiz_gen = QuizGen(namespace=st.session_state['selected_chapter']['id'])
                quiz_bank = quiz_gen.generate(10)
                st.session_state['quiz_bank'] = quiz_bank
                st.success("Quiz bank generated successfully!")
                for quiz in quiz_bank:
                    st.write("Source: ", quiz['quiz_src'])
                    st.json(quiz['quiz_formatted'])

    if st.session_state['quiz_bank'] is not None:
        delete_bank = st.button("Delete Quiz Bank")
        if delete_bank:
            if st.session_state['quiz_bank'] is None:
                st.warning("No quiz bank to delete.")
            else:
                st.session_state['quiz_bank'] = None
                st.success("Quiz bank deleted successfully!")